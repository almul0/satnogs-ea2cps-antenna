...
Los parametros de la antena se obtendrán de las condiciones de diseño del modo axial descritas en la siguiente table:  \ref{table:condiciones_diseno}
...
\begin{tabular}{|c|c|}
	...
	Parámetro & Valor \\ 
	\hline 
	Circunferencia & $\frac{3\lambda}{4} < C < \frac{4\lambda}{3}$  \\ 
	\hline 
	...
	N & $3 < N < 15$  \\ 
	\hline 
	Diámetro del plano de tierra & $D > \frac{\lambda}{2}$ \\ 
	\hline 
	Diámetro del conductor (t)	& $ 0.001\lambda < t < 0.01 \lambda$   \\ 
	\hline 
	Impedancia de entrada & Sensible a la alimentación. Entre 140 y 250 $\Omega$  \\ 
	\hline 
\end{tabular}
\caption{Condiciones de diseño para antena helicoidal modo axial}
\label{table:condiciones_diseno}
\end{table} 

Para un diseño óptimo $C \simeq \lambda_0$ y $\alpha \simeq 13°$ con estos parámetros fijados se obtiene una separación entre vueltas:
\begin{equation}
S = C \tan{\alpha} = \lambda_0 \tan(13°) = 0.231 \lambda_0 
\end{equation}
La longitud de una vuelta entonces seǵun ECUACION O FIGURA DE DIEGO:
\begin{equation}
L_0=\sqrt{S^2 + C^2} = \lambda_0 \sqrt{(0.231)^2 + (1)^2} = 1.0263 \lambda_0
\end{equation}
Con estas condiciones de diseño impuestas se procederá a su simulación mediante el software de 4nec2. Dicho software calcula mediante el método de momentos la respuesta electromágnetica de antenas y estructuras metálicas. El software viene con un generador de geometrías que se puede utilizar para generar un archivo .nec con los segmentos del reflector o la hélice, aunque la hélice se puede generar con un tipo especial de hilo en el mismo editor del programa.

Se empezará simulando la hélice simple sin plano de tierra para una frecuencia de 435 MHz ($\lambda_0 = 68.965cm$)
